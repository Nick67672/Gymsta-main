...
//       })
      ...
      Alert.alert(
        'Error',
        e?.message ? String(e.message) : 'Could not add entry.'
      );
    }
  };
  ...
  const renderItem = ({ item }: { item: FoodDefinition }) => (
    <View style={[styles.row, { borderColor: colors.border }]}>
      <View style={{ flex: 1 }}>
        ...
        >{`${Math.round(item.calories)} kcal / 100g • P${item.protein_g} C${
          item.carbs_g
        } F${item.fat_g}`}</Text>
        <View style={styles.servingsWrap}>
          {item.common_servings.slice(0, 3).map((s) => (
            <TouchableOpacity
              key={s.label}
              onPress={() => setSelectedServing(s.grams)}
              style={[
                styles.servingChip,
                {
                  borderColor: colors.border,
                  backgroundColor:
                    selectedServing === s.grams ? colors.tint : 'transparent',
                },
              ]}
            >
              <Text
                style={{
                  color:
                    selectedServing === s.grams ? '#fff' : colors.textSecondary,
                  fontSize: 12,
                }}
              >
                {s.label}
